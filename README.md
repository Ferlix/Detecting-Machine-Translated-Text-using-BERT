...
- 3. (OPTIONAL) If you would like to generate a dataset yourself follow these steps:
  ...
  - 3. create_chunks.py will generate chunks of the source language, as well as their aligned human translations in separate chunks.
  - 4. Select random chunks from the source language and translate using an online machine translator. 
  - 5. Use create_dataset.py to merge the translated chunks as well as to merge it with the human translated data in   
merge chunks and add corresponding labels.
  - 6. Now a dataset is obtained which should be split into a training and testing set as desired and the eventual files should be .tsv files. The dataset structure is a human translated subtitle, followed by its machine translated counterpart. 
  - 7. In order to visualize the data you created you can use the jupyter notebook "plotting_script.ipynb" in the main folder of this repo.
